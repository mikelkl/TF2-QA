...
def read_simplified_annotation(gold_path):
    annotation_dict = {}
    with open(gold_path) as f:
        for line in f:
            ...
            for annotation in json_example['annotations']:
                long_span_rec = annotation['long_answer']
                long_span = Span(-1, -1, long_span_rec['start_token'], long_span_rec['end_token'])

                short_span_list = []
                for short_span_rec in annotation['short_answers']:
                    short_span = Span(-1, -1, short_span_rec['start_token'], short_span_rec['end_token'])
                    short_span_list.append(short_span)
