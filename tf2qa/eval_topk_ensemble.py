...
def evaluate(model, args, dev_features, device, ei):
    # Eval!
    if os.path.exists(os.path.join(args.output_dir, 'RawResults_ensemble{}.pkl'.format(ei))):
        all_results = pickle.load(open(os.path.join(args.output_dir, 'RawResults_ensemble{}.pkl'.format(ei)), 'rb'))
    else:
        all_results = []
        ...
        pickle.dump(all_results, open(os.path.join(args.output_dir, 'RawResults_ensemble{}.pkl'.format(ei)), 'wb'))
    ...
    output_prediction_file = os.path.join(args.output_dir, 'predictions{}.json'.format(ei))
...
if __name__ == '__main__':
    ...
    all_preds = glob(args.output_dir + '/predictions*.json')
    ensemble_pred_dict = {}
    ...
    output_prediction_file = os.path.join(args.output_dir, 'predictions_ensemble.json')
    with open(output_prediction_file, 'w') as f:
